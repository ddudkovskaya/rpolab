...
Java_com_example_lab1_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
    std::string hello = "Hello from Dasha";
...
Java_ru_iu3_lab1_MainActivity_initRng(JNIEnv *env, jclass clazz) {
...
Java_com_example_lab1_MainActivity_randomBytes(JNIEnv *env, jclass clazz, jint no) {
    uint8_t * buf = new uint8_t [no];
    mbedtls_ctr_drbg_random(&ctr_drbg, buf, no);
    jbyteArray rnd = env->NewByteArray(no);
    env->SetByteArrayRegion(rnd, 0, no, (jbyte *)buf);
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_example_lab1_MainActivity_encrypt(JNIEnv *env,
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_example_lab1_MainActivity_decrypt(JNIEnv *env,
                                          jclass, jbyteArray key, jbyteArray data)
{
    jsize ksz = env->GetArrayLength(key);
    jsize dsz = env->GetArrayLength(data);
